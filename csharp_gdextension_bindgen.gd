...
const GENERATED_NAMESPACE = "GDExtension"
const GENERATED_SCRIPTS_FOLDER = "res://addons/csharp_gdextension_bindgen/Generated"
...
const ENUM_TYPE_OVERRIDES = {
	"CanvasItem.HorizontalAlignment": "Godot.HorizontalAlignment",
	"Node3D.EulerOrder": "Godot.EulerOrder",
}

const FIRST_GEN_HINT = "res://.godot/csharp_gdextension_bindgen"

func _enter_tree():
	add_tool_menu_item(MENU_ITEM_NAME, generate_gdextension_csharp_scripts)
	if not FileAccess.file_exists(FIRST_GEN_HINT):
		var f = FileAccess.open(FIRST_GEN_HINT, FileAccess.ModeFlags.WRITE)
		f.store_string("")
		f.close()
		generate_gdextension_csharp_scripts()
...
static func generate_csharp_script(
	...
	var parent_class_type = _get_csharp_class_name(parent_class)
	var engine_class_type = _get_csharp_class_name(engine_class)

	var regions = PackedStringArray()

	# Engine object used for calling engine methods
	if not parent_class_is_extension:
		regions.append("// Engine object used for calling engine methods\nprotected %s _object;" % parent_class_type)

	# Constructors
	var ctor_fmt
	if parent_class_is_extension:
		ctor_fmt = """
			public {cls_name}([NotNull] {engine_class} @object) : base(@object)
			{
			}
		"""
	else:
		ctor_fmt = """
			public {cls_name}([NotNull] {engine_class} @object)
			{
				_object = @object;
			}
		"""
	var ctor = ctor_fmt.dedent().format({
		cls_name = cls_name,
		engine_class = engine_class_type,
	}).strip_edges()
	regions.append(ctor)
	...
	var casts = """
		...
		public static explicit operator {cls_name}(Variant variant) => new(({engine_class})variant);
	""".dedent().format({
		cls_name = cls_name,
		engine_class = engine_class_type,
	}).strip_edges()
	...
	for enum_name in ClassDB.class_get_enum_list(cls_name, true):
		var enum_code = _generate_enum(cls_name, enum_name)
		if not enum_code.is_empty():
...
static func generate_gdextension_csharp_scripts(
	...
	print("C# bindings for gdextension generated.")

static func _generate_enum(cls_name: StringName, enum_name: StringName) -> String:
	var enum_constants = ClassDB.class_get_enum_constants(cls_name, enum_name, true)
	...
	if common_prefix in enum_constants:
...
static func _generate_property(cls_name: StringName, property: Dictionary) -> String:
	...
	var csharp_property_name_cache = property_name.to_pascal_case()
	var csharp_property_name = csharp_property_name_cache
	if csharp_property_name in ["PropertyName", "MethodName", "SignalName"]:
		csharp_property_name += "Property"
	...
	if getter:
		if _is_extension_class(cls_name):
			getset.append("get => {get_cast}_object.Get(PropertyName.{csharp_property_name});".format({
				...
				csharp_property_name = csharp_property_name_cache,
			}))
		else:
	...
	match property["type"]:
		...
		TYPE_OBJECT:
			if property["class_name"] and property["class_name"] != "Object":
				return _get_csharp_class_name(property["class_name"])
			else:
				return "GodotObject"
		TYPE_ARRAY:
...
static func _get_array_element_type(cls_name: StringName, hint_string: String) -> String:
	var hint_parts = hint_string.split(":", true, 1)
	var type_parts = hint_parts[0].split("/", true, 1)
	var variant_type = int(type_parts[0])
	var nested_hint = hint_parts[1] if hint_parts.size() > 1 else ""
	match variant_type:
		TYPE_NIL:
			return ""
		TYPE_INT:
			if not nested_hint.is_empty():
				var split = nested_hint.split(".")
				if split.size() == 1:
					return _get_enum_type(cls_name, cls_name, nested_hint)
				return _get_enum_type(cls_name, split[0], split[1])
			return "int"
		TYPE_OBJECT:
			if nested_hint.is_empty() or nested_hint == "Object":
				return "GodotObject"
			if _is_extension_class(nested_hint):
				return _get_csharp_class_name(_first_non_extension_parent(nested_hint))
			return _get_csharp_class_name(nested_hint)
		_:
			return _get_mapped_variant_type(type_string(variant_type))


static func _get_enum_type(context_cls_name: StringName, enum_owner_cls_name: StringName, enum_name: String) -> String:
	var qualified_enum_name = "%s.%s" % [enum_owner_cls_name, enum_name]
	if ENUM_TYPE_OVERRIDES.has(qualified_enum_name):
		return ENUM_TYPE_OVERRIDES[qualified_enum_name]
	var enum_type = "%s%s" % [enum_name, "Enum" if _needs_enum_suffix(enum_owner_cls_name, enum_name) else ""]
	if enum_owner_cls_name == context_cls_name and _is_extension_class(enum_owner_cls_name):
		return enum_type
	return "%s.%s" % [_get_csharp_class_name(enum_owner_cls_name), enum_type]


static func _get_csharp_class_name(cls_name: String) -> String:
	var resolved_cls_name = _get_class_from_class_name(cls_name)
	if _is_extension_class(resolved_cls_name):
		return resolved_cls_name
	return _pascal_to_pascal_case(resolved_cls_name)
...
static func _get_method_return_type(cls_name: StringName, method_name: StringName, method_return: Dictionary) -> String:
	...
	if method_name in [
		...
		return "Godot.Collections.Array<Godot.Collections.Dictionary>"
	if method_name in ["get_meta_list", "get_groups"]:
		return "Godot.Collections.Array<StringName>"
...
static func _generate_strings_class(cls_name: StringName, string_name_type: StringNameType, string_names: PackedStringArray) -> String:
	...
	""".dedent().format({
		...
		parent_class = _get_csharp_class_name(parent_class),
